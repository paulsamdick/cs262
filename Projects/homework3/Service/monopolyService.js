...
function readHelloMessage(req, res) {
    res.send('Hello, CS 262 Monopoly service Pauls updates!');
}
...
function readJoin(req, res, next) {
    db.many("SELECT ID, gameID, score FROM Player INNER JOIN PlayerGame ON Player.ID = PlayerGame.playerID LIMIT 1")
    .then(data => {
        res.send(data);
    })
    .catch(err=> {
